<x-app-layout> {{ __('') }} <flux:heading level="1" class="mb-2">{{ __('Update Project') }}</flux:heading>
            <flux:text class="text-zinc-600 dark:text-zinc-400 mb-8">{{ __('Update project details and audit settings.') }}</flux:text>

            <form action="{{ route('accessibility-projects.update', $project) }}" method="POST" enctype="multipart/form-data"> {{ __('') }} <flux:label>{{ __('Project Name') }}</flux:label>
                        <flux:input 
                            type="text" 
                            name="name" 
                        ...
                        <flux:textarea 
                            name="description" 
                            rows="4"
                        >{{ old('description', $project-> {{ __('') }} </flux:field>

                    <div>
                        <flux:heading size="sm" level="3" class="mb-3">{{ __('Client Logo') }}</flux:heading>
                        @if ($project-> {{ __('') }} <img src="{{ Storage::url($project->client_logo) }}" alt="{{ __('Client logo') }}" class="max-h-10 max-w-full object-contain" /> {{ __('') }} <input type="checkbox" name="remove_client_logo" value="1" class="rounded" />
                        ...
                        <flux:field>
                            ...
                            <flux:description>{{ __('Optional. Shown on reports. PNG, SVG or JPG, max 2 MB.') }}</flux:description> {{ __('') }} </div>
                    ...
                    <flux:field>
                        ...
                        >
                            ...
                            <option value="AAA" {{ old('target_wcag_level', $project->target_wcag_level) === 'AAA' ? 'selected' : '' }}>{{ __('WCAG 2.1 Level AAA') }}</option> {{ __('') }} </flux:field>
                    ...
                    <flux:field>
                        ...
                        >
                            ...
                            <option value="completed" {{ old('status', $project->status) === 'completed' ? 'selected' : '' }}>{{ __('Completed') }}</option> {{ __('') }} </flux:field>
