<x-app-layout>
    <div class="min-h-screen bg-white dark:bg-zinc-900">
        <div class="max-w-2xl mx-auto px-4 py-8">
            <flux:heading level="1" class="mb-2">Edit Project</flux:heading>
            <flux:text class="text-zinc-600 dark:text-zinc-400 mb-8">Update project details and audit settings.</flux:text>
            ...
            <form action="{{ route('accessibility-projects.update', $project) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <flux:heading size="sm" level="3" class="mb-3">Client Logo</flux:heading>
                        @if ($project->client_logo)
                            <div class="flex items-center gap-4 mb-3">
                                <div class="rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 p-3 flex items-center justify-center w-32 h-16">
                                    <img src="{{ Storage::url($project->client_logo) }}" alt="Client logo" class="max-h-10 max-w-full object-contain" />
                                </div>
                        ...
                        @endif
                        <flux:field>
                            <flux:label>{{ $project->client_logo ? 'Replace logo' : 'Upload client logo' }}</flux:label>
                            <flux:input type="file" name="client_logo" accept="image/*" />
                            <flux:description>Optional. Shown on reports. PNG, SVG or JPG, max 2 MB.</flux:description>
