...
if os.path.exists(LANG):
    with open(LANG, encoding='utf-8') as f:
        try:
            ...
            lang = {}
changed_files = set()
pattern_line = re.compile(r'^(?P<file>[^:]+):(?P<line>\d+): (?P<text>.*) -> (?P<tag>\w+)$')
with open(IN, encoding='utf-8') as fh:
    for line in fh:
        ...
        if 'database/seeders' in filepath or filepath.startswith('database/seeders'):
            ...
            continue
        # skip content-like constructs
        if any(s in text for s in ['{{', '{!!', '$', '@', 'http://', 'https://']):
            print('Skipping unsafe text (blade/variable/url):', filepath, lineno, text)
            continue
        # process only translate or stub per user request (everything except seeders)
        if tag not in ('translate','stub'):
            print('Skipping tag', tag, 'for', filepath)
            continue
        ...
        def repl(m):
            ...
            return m.group(0)
        new_line, n = inner_pat.subn(repl, orig_line, count=1)
        if n>0 and new_line != orig_line:
            ...
            print('No exact >text< match on line, skipping:', filepath, lineno, text)
# write lang file if modified
if changed_files:
...
if changed_files:
    ...
    subprocess.run(['git','commit','-m','i18n: wrap translate/stub-tagged text nodes in views (exclude seeders)','-m','Auto-wrapped translate/stub-tagged text nodes in views and examples; seeders excluded.'], check=False)
